...
***
  ...
  # Overview
  This script fits log-normal Generalized Additive Models (GAMs) and Autoregressive Integrated Moving Average (ARIMA) models to salmon run-size data with or without regression covariates. These models are then used to generate inseason abundance forecasts. The data used to fit the models is split into "training" and "validation" sets to evaluate performance of the forecasts, compare models, and develop weighted "ensemble" forecasts. The code repository used to generate this page and complete analyses can be found here: [**(link)**](https://github.com/tbuehrens/Salmon_Forecast_Example)
...
wd_functions<-"C:/Users/bogmr477/OneDrive - Washington State Executive Branch Agencies/Documents/Chum/ISU/Salmon_Forecast_Example/functions"
...
# Load Data
Here we will load and format our data for analysis. First, we will load and format our fish data for analysis. 
```{r load_data, message = FALSE, warning = FALSE,results = "show"}
#==========================
#get Chum data 
#==========================
## read total runsize data
setwd("C:/Users/bogmr477/OneDrive - Washington State Executive Branch Agencies/Documents/Chum/ISU/Salmon_Forecast_Example/data")

dat_trs <- read_csv("chum_trs.csv")%>%
  dplyr::select(Year=year,runsize_obs = AllSS_runsize)%>%
  filter(!is.na(runsize_obs))
## read in test fishery data
dat_tst_fsh <- read_csv("chum_CPUE_cov_22.csv")%>%
  mutate(date = mdy(date))%>%
  dplyr::rename(Year=year)
#look at CPUE data to evaluate consistency
dat_tst_fsh%>%
  ...
  print(n=100)
#Construct dataframe, combining run size data with test fishing data. We select the date range based on the ava
...
ggplot(data=forecasts,aes(x=Year,y=Estimate,color=Model))+
  ...
  ylim(0,NA)+xlim(2011,2021)+
  theme_bw()
```
#write.csv(forecasts, "Week2022_Forecasts.csv")
# Evaluate Forecasts
...
forecast_skill%>%
  ...
  kable_classic(full_width = F, html_font = "Cambria")
```
#write.csv(forecast_skill, "2022_Forecast_skill.csv")
...
best_model%>%
  ...
  kbl(caption = "Week 44 ensemble forecast 2022 South Sound chum abundance",digits =0)%>%
  kable_classic(full_width = F, html_font = "Cambria")
```
##write.csv(best_model, "Week42_top5_best_model.csv")
